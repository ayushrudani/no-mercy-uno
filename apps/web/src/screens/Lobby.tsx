      ...
      <div className="mx-auto mt-6 grid max-w-3xl gap-4 sm:grid-cols-2">
        ...
        <section className="panel panel-raised rounded-2xl p-4">
          ...
          <label className="mt-4 flex cursor-pointer items-start justify-between gap-3 rounded-xl bg-white/4 px-3 py-2.5 ring-1 ring-white/8">
            <span>
              <span className="block text-xs font-semibold">Knock out at 25 cards</span>
              <span className="mt-0.5 block text-[10px] leading-snug text-white/40">
                Official No Mercy. Hit 25 cards and you are out for good, ranked below
                everyone who emptied their hand. Off by default.
              </span>
            </span>
            <input
              type="checkbox"
              checked={(settings.rules.eliminationAt ?? 0) > 0}
              onChange={(e) =>
                setSettings({
                  ...settings,
                  rules: { ...settings.rules, eliminationAt: e.target.checked ? 25 : 0 },
                })
              }
